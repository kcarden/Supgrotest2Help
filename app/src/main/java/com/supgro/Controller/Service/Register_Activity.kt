...
class Register_Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(com.supgro.R.layout.register_activity_layout)
    ...
    private fun performRegister() {
       ...
       //val struggle = struggletext.text.toString()
        ...
        }else if(!comfirmPasswordRegEdTxt.getText().toString().equals(comfirmPasswordRegEdTxt.getText().toString()) )
        {
            Toast.makeText(this, "Please make sure email and password match", Toast.LENGTH_SHORT).show()

        }else {
            Toast.makeText(baseContext,"You have successfully created an account!",Toast.LENGTH_SHORT).show()
        ...
        FirebaseAuth.getInstance().createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener {
                ...
                //else
                Log.d("RegisterActivity", "Sucessfully created uid: ${it.result?.user?.uid}")
...
*/
                    ...
                    FirebaseDatabase.getInstance().reference.child("Support").child(currentUserID)
                        .child("Supporting").child("HdzG4I3kzEcS1wNlWYhHfBspF8H3")
                        .setValue(true)
  ...
  /* private fun addsupgroassupporter(){
       val currentUserID = FirebaseAuth.getInstance().currentUser!!.uid
    ...
    supportingRef.addValueEventListener(object: ValueEventListener {
        ...
        override fun onDataChange(p0: DataSnapshot) {
            if (p0.exists()){
                supporters_Count_txt?.text = p0.childrenCount.toString()
            }
        }
